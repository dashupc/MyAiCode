...
class MSSQLBackupTool:
    def __init__(self, root, silent_mode=False):
        ...
        self.root.title("MSSQL数据库备份工具 - V0.38（紧凑配置版）")
        ...
        try:
            ...
            if os.path.exists(self.icon_path):
                try:
                    ...
                    self.log(f"已加载图标文件: {self.icon_path}")
                except Exception as e:
                    self.log(f"加载图标文件失败: {str(e)}，将使用默认图标")
                    self.common_icon = None
            
            # 如果没有有效的图标文件，创建默认图标
            if self.common_icon is None:
            ...
            self.log(f"设置图标失败: {str(e)}")

    def init_tray_icon(self):
        """使用pystray初始化系统托盘，确保使用统一图标"""
        ...
        try:
            ...
            menu = (
                ...
                item('清理本地备份', self.tray_action_manual_cleanup),
                item('清理服务器备份', self.tray_action_manual_server_cleanup),
    ...
    def show_window(self):
        ...
        try:
            ...
            self.root.lift()  # 置顶窗口
            self.root.attributes('-topmost', True)  # 强制置顶
            self.root.focus_force()  # 获取焦点
            # 短暂延迟后取消强制置顶，避免一直遮挡其他窗口
            self.root.after(1000, lambda: self.root.attributes('-topmost', False))
            self.log("从系统托盘恢复窗口显示（增强版）")
        except Exception as e:
    ...
    def setup_logging(self):
        logging.basicConfig(
            filename='backup_log_v0.38.txt',
            level=logging.INFO,
            format='%(asctime)s - %(levelname)s - %(message)s',
            encoding='utf-8'
        )
        self.log("程序启动（版本V0.38 - 紧凑配置版）")
    ...
    def create_log_tab(self):
        ...
        log_display_frame = ttk.LabelFrame(log_frame, text="操作日志（版本V0.38 - 紧凑配置版）", padding="10")
    ...
    def create_monitor_tab(self):
        ...
        # 系统状态区域 - 分为两列显示
        status_frame = ttk.LabelFrame(monitor_frame, text="系统状态（版本V0.38 - 紧凑配置版）", padding="10")
    ...
    def export_log(self):
        try:
            ...
            default_filename = f"backup_log_export_{timestamp}_V0.38.txt"
    ...
    def save_config(self):
        try:
            ...
            config_file = 'backup_config_v0.38.json'
            with open(config_file, 'w', encoding='utf-8') as f:
                json.dump(config, f, ensure_ascii=False, indent=4)
            
            self.log(f"配置已保存，备份列表中包含 {len(self.selected_databases)} 个数据库: {', '.join(self.selected_databases)}")
    ...
    def load_config(self):
        try:
            ...
            if not os.path.exists(config_file):
                for old_version in ['v0.37', 'v0.36', 'v0.35']:
    ...
    def delete_old_backups(self):
        ...
        try:
            ...
            self.log(f"开始自动清理本地备份（版本V0.38）：删除 {days} 天前的备份文件")
