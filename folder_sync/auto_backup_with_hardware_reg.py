...
class HardwareRegistrationSystem:
    ...
    def get_motherboard_serial(self):
        """获取主板序列号"""
        try:
            self.debug_info.append("获取主板序列号...")
            # 使用WMI获取硬件信息
            c = wmi.WMI()
            for board in c.Win32_BaseBoard():
                # 返回主板序列号（去除空格并转为大写）
                serial = board.SerialNumber.strip().upper()
                self.debug_info.append(f"获取到主板序列号: {serial}")
                return serial if serial else self._generate_fallback_serial()
        except Exception as e:
            error_msg = f"获取主板序列号失败: {str(e)}"
            self.debug_info.append(error_msg)
            # 失败时返回一个随机字符串作为 fallback
            return self._generate_fallback_serial()
    
    def _generate_fallback_serial(self):
        """生成备用序列号（当无法获取硬件信息时）"""
        fallback = 'FALLBACK-' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=12))
        self.debug_info.append(f"生成备用序列号: {fallback}")
        return fallback
    
    def generate_machine_code(self, motherboard_serial=None):
        """根据主板序列号生成机器码"""
        self.clear_debug_info()
        self.debug_info.append("开始生成机器码...")
        
        # 如果未提供序列号，自动获取
        if not motherboard_serial:
            motherboard_serial = self.get_motherboard_serial()
        else:
            self.debug_info.append(f"使用提供的主板序列号: {motherboard_serial}")
        
        # 对序列号进行哈希处理生成机器码
        self.debug_info.append(f"对序列号进行SHA-256哈希: {motherboard_serial}")
        hash_obj = hashlib.sha256(motherboard_serial.encode('utf-8'))
        hash_hex = hash_obj.hexdigest()
        self.debug_info.append(f"SHA-256哈希结果: {hash_hex}")
        
        # 取前20位
        hash_part = hash_hex[:20]
        self.debug_info.append(f"取前20位: {hash_part}")
        
        # 格式化为XXXX-XXXX-XXXX-XXXX-XXXX
        machine_code = '-'.join([hash_part[i:i+4] for i in range(0, 20, 4)])
        self.debug_info.append(f"生成机器码: {machine_code}")
    ...
    def generate_registration_key(self, machine_code):
        ...
        if len(cleaned_machine_code) != 20:
            raise ValueError(f"机器码格式错误，应为20位字符，实际为{len(cleaned_machine_code)}位")
    ...
    def verify_registration_key(self, machine_code, registration_key):
        """验证注册码 - 与生成逻辑严格匹配"""
        self.debug_info.append("\n开始验证注册码...")
        self.debug_info.append(f"待验证注册码: {registration_key}")
        self.debug_info.append(f"当前机器码: {machine_code}")
        
        # 基本格式检查
        if not registration_key:
            self.debug_info.append("验证失败: 注册码为空")
            return False
            
        if len(registration_key) != 24 or registration_key.count('-') != 4:
            self.debug_info.append(f"验证失败: 注册码格式错误，长度为{len(registration_key)}，横线数为{registration_key.count('-')}")
            return False
            
        if not machine_code or len(machine_code) != 24 or machine_code.count('-') != 4:
            self.debug_info.append(f"验证失败: 机器码格式错误，长度为{len(machine_code)}，横线数为{machine_code.count('-')}")
            return False
            
        try:
            # 生成对应机器码的注册码并比较
            generated_key = self.generate_registration_key(machine_code)
            self.debug_info.append(f"生成的正确注册码: {generated_key}")
            
            if generated_key == registration_key:
                self.debug_info.append("验证成功: 注册码匹配")
                return True
            else:
                self.debug_info.append("验证失败: 注册码不匹配")
                return False
        except Exception as e:
...
class BackupTool:
    def __init__(self, root, start_minimized=False):
        ...
        self.REGISTRATION_KEY = "RegistrationKey"  # 注册表键名
        self.MACHINE_CODE_KEY = "MachineCode"      # 存储机器码
    ...
    def check_license_status(self):
        ...
        try:
            ...
            self.log(f"生成机器码: {self.machine_code}")
            ...
            self.log(f"从注册表读取到注册码: {registered_value}")
            
            # 验证注册码是否与当前机器匹配
            self.reg_system.clear_debug_info()
            ...
            self.log("注册表中未找到注册信息")
        except Exception as e:
            self.log(f"检查注册状态时出错: {str(e)}")
    ...
    def check_trial_period(self):
        ...
        try:
            ...
            try:
                ...
                winreg.CloseKey(key)
                self.log("已保存试用期信息到注册表")
            except Exception as e:
                self.log(f"保存试用期信息失败: {str(e)}")
    ...
    def show_registration_window(self):
        ...
        if self.trial_end_date is not None and datetime.now() > self.trial_end_date:
            ttk.Label(trial_frame, text="试用期已结束，请输入注册码以继续使用。", 
                    font=("SimHei", 10), foreground="red").pack(anchor=tk.W)
        else:
            days_left = (self.trial_end_date - datetime.now()).days + 1
            ttk.Label(trial_frame, text=f"试用期剩余 {days_left} 天，请输入注册码以解锁全部功能。", 
        ...
        def register():
            ...
            if len(key) != 24 or key.count('-') != 4:
                status_var.set(f"注册码格式错误，应为24位字符(包含4个横线)，实际为{len(key)}位")
                return
                
            try:
                # 清除之前的调试信息并验证注册码
                self.reg_system.clear_debug_info()
                is_valid = self.reg_system.verify_registration_key(self.machine_code, key)
                ...
                if is_valid:
                    ...
                    try:
                        ...
                        status_var.set(f"保存注册信息失败: {str(e)}")
                else:
                    status_var.set("注册码无效，请检查是否与机器码匹配")
            except Exception as e:
                status_var.set(f"验证过程出错: {str(e)}")
...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1] == "--generate-key":
        ...
        generate_registration_key_for_machine(machine_code)
        return
